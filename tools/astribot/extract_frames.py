"""Extract sub-task splits, key-frame jpgs and/or per-subtask videos from a
local LeRobotDataset copy.

Three independent modes:

- split_subtask: detects the gripper key frames and infers the sub-task split
  frames from the tabular observation.state only — no video is read, so it
...
Examples
--------
    # 1) infer the sub-task split frames of every episode (no videos needed)
    python tools/astribot/extract_frames.py \
        --repo-id Kronze157/astri_making_coffee_vlva \
        --data-root /data/astri_making_coffee --mode split_subtask

    # 2) save the split frames as jpgs for the first two cameras
    python tools/astribot/extract_frames.py \
        --repo-id Kronze157/astri_making_coffee_vlva \
        --data-root /data/astri_making_coffee --mode extract_frames \
        --camera-idxes 0 1

    # 3) one mp4 per sub-task segment, first two cameras
    python tools/astribot/extract_frames.py \
        --repo-id Kronze157/astri_making_coffee_vlva \
        --data-root /data/astri_making_coffee --mode video \
        --camera-idxes 0 1
"""
...
MODE_ROOTS = {"split_subtask": "subtask",
              "extract_frames": "key_frames",
              "video": "subtask_videos"}
...
def parse_args():
    ...
    parser.add_argument("--mode", "-m", required=True,
                        choices=("split_subtask", "extract_frames", "video"),
                        help="split_subtask: detect the gripper key frames and infer the "
                             "sub-task split frames from observation.state only (no video "
                             "access), saving the gripper plot and subtask_splits.json; "
                             "extract_frames: save one jpg per split frame per camera "
                             "(from subtask_splits.json); "
                             "video: one mp4 per sub-task segment per camera "
                             "(from subtask_splits.json)")
...
class DataExtract:
    ...
    @staticmethod
    def _to_bgr(img):
        # PIL is RGB; cv2.VideoWriter expects BGR. Depth (single-channel)
        # frames arrive as 2D arrays and are written as grayscale by
    ...
    def _output_dirs(self):
        ...
        the per-episode dir out_dir/<mode_root>/ep{ep}/ (see MODE_ROOTS and
        _episode_dir)."""
        frame_dirs, video_dirs = {}, {}
        for idx, cam_key in self.cam_keys.items():
            sub = self._camera_subdir(cam_key)
            if self.args.mode == "extract_frames":
                frame_dirs[idx] = sub
    ...
    def _ensure_dataset(self):
        ...
        extract_frames mode decodes videos (the video mode cuts them with
        ffmpeg straight from disk)."""
    ...
    def _subtask_split_idxes(self, frames, vals, smooth=None):
        ...
        (no preceding 1) or end (no return to 1) yields no split. The
    ...
    def _split_file_path(self):
        ...
        split_subtask, read by extract_frames and video). Always lives
        under the subtask root, so the reading modes can find it without
        knowing the split_subtask layout."""
        return os.path.join(self.out_dir, MODE_ROOTS["split_subtask"],
    ...
    def _load_splits(self):
        ...
        if not os.path.isfile(path):
            raise FileNotFoundError(
                f"{path} missing: run --mode split_subtask first "
                "(--dedup-tasks skips episodes whose task already has output)")
        with open(path) as f:
            return json.load(f)

    def _plot_gripper_state(self, frames, vals, subtask_idxes=None):
        ...
        _, ymax = ax.get_ylim()
        for k in self._key_frame_idxes(frames, vals, smooth):
        ...
        splits = self._subtask_split_idxes(frames, vals, smooth)
        if splits:
            ymin, _ = ax.get_ylim()
            for s in splits:
    ...
    def _save_split_frames(self, split_idxes):
        """Save the sub-task split frames as one jpg per selected camera under
        <out_dir>/key_frames/<episode>/<camera>/."""
        base = self._episode_dir()
        for ci, sub in self.frame_dirs.items():
            os.makedirs(os.path.join(base, sub), exist_ok=True)
        ds = self._ensure_dataset()
        for frame_idx in split_idxes:
            frame = ds[frame_idx]
            for ci, sub in self.frame_dirs.items():
                name = f"frame_{frame_idx:06d}.jpg"
                img = to_pil(frame[self.cam_keys[ci]])
                if "depth" in self.cam_keys[ci]:
                    # depth streams are stored as color RGB; save grayscale
                    img = self._to_gray_pil(img)
                img.save(os.path.join(base, sub, name))
    ...
    def _write_subtask_videos(self, split_idxes):
        ...
        depth cameras are written as monochrome streams (see _to_gray_pil).
        """
        ...
        for ci, sub in self.video_dirs.items():
            ...
            for seg_i, (lo, hi) in enumerate(zip(bounds, bounds[1:])):
    ...
    def _process_episode(self, ep_idx):
        ...
        self.middle_idx = (self.from_idx + self.to_idx - 1) // 2
        df = self._load_episode_table()
        ...
        if self.args.mode == "split_subtask":
            ...
            if self.gripper_idxes:
                self._plot_gripper_state(frames, vals, subtasks)
            self._save_splits(key_idxes, split_idxes)
        elif self.args.mode == "extract_frames":
            self._save_split_frames(self._load_splits()["split_frames"])
        else:  # video
