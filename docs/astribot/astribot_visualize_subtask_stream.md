...
from the saved depth_pose outputs, and the colour images shown in the point clouds are
...
| `--camera-idxes`, `-c` | first non-depth camera | indices into the dataset's `camera_keys`; must match the cameras used by `run_step2_depth_stream.py` |
...
| `--out-dir`, `-o` | `<data-root>/eps_data` | output root; results read from `<out-dir>/depth_pose/<episode>/subtask_XX/` |
| `--use-inferred-splits` | off | prefer the split frames inferred by `detect_subtask` (`subtask_splits.json`) over the ground-truth `subtask_index` column |
