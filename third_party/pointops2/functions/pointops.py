...
class AttentionStep1(Function):
    ...
    def forward(ctx, q, k, index0, index1):
        ...
        output = torch.zeros(M, h, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_q = torch.zeros(N_q, h, C//h, dtype=torch.float32, device='cuda')
        grad_k = torch.zeros(N_k, h, C//h, dtype=torch.float32, device='cuda')
...
class AttentionStep2(Function):
    ...
    def forward(ctx, attn, v, index0, index1):
        ...
        output = torch.zeros(N_q, h, C//h, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_attn = torch.zeros(M, h, dtype=torch.float32, device='cuda')
        grad_v = torch.zeros(N_v, h, C//h, dtype=torch.float32, device='cuda')
...
class AttentionStep2_v2(Function):
    ...
    def forward(ctx, attn, v, index0, index1):
        ...
        output = torch.zeros(L, h, C//h, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_attn = torch.zeros(M, h, dtype=torch.float32, device='cuda')
        grad_v = torch.zeros(N, h, C//h, dtype=torch.float32, device='cuda')
...
class DotProdWithIdx(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_q = torch.zeros(N, h, hdim, dtype=torch.float32, device='cuda')
        grad_table = torch.zeros(L, h, hdim, 3, dtype=torch.float32, device='cuda')
...
class DotProdWithIdx_v2(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_q = torch.zeros(N, h, hdim, dtype=torch.float32, device='cuda')
        grad_table_q = torch.zeros(L, h, hdim, 3, dtype=torch.float32, device='cuda')
        grad_k = torch.zeros(N, h, hdim, dtype=torch.float32, device='cuda')
        grad_table_k = torch.zeros(L, h, hdim, 3, dtype=torch.float32, device='cuda')
...
class AttentionStep2WithRelPosValue(Function):
    ...
    def forward(ctx, attn, v, index0, index1, table, rel_idx):
        ...
        output = torch.zeros(N_q, h, hdim, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_attn = torch.zeros(M, h, dtype=torch.float32, device='cuda')
        grad_v = torch.zeros(N_v, h, hdim, dtype=torch.float32, device='cuda')
        grad_table = torch.zeros(L, h, hdim, 3, dtype=torch.float32, device='cuda')
...
class AttentionStep2WithRelPosValue_v2(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index1, table, rel_idx):
        ...
        output = torch.zeros(N, h, hdim, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_attn = torch.zeros(M, h, dtype=torch.float32, device='cuda')
        grad_v = torch.zeros(N, h, hdim, dtype=torch.float32, device='cuda')
        grad_table = torch.zeros(L, h, hdim, 3, dtype=torch.float32, device='cuda')
